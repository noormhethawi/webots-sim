...
# Step 1: Position the arm above the box
print("Positioning arm above the box...")
arm_motors[0].setPosition(0.0)  # Ensure base is facing forward
arm_motors[1].setPosition(-0.5)  # Lower shoulder to box level
arm_motors[2].setPosition(-1.2)  # Extend elbow forward
arm_motors[3].setPosition(0.0)   # Neutral wrist position
wait_until_motor_reaches(arm_sensors[2], -1.2)

# Step 2: Lower the arm to the ground level (reach the box)
print("Lowering the arm towards the box...")
arm_motors[1].setPosition(-1.2)  # Lower shoulder to bring the arm closer to the ground
arm_motors[2].setPosition(-2.0)  # Extend elbow further to make the arm reach the ground
wait_until_motor_reaches(arm_sensors[2], -2.0)
...
arm_motors[1].setPosition(0.0)  # Lift shoulder
arm_motors[2].setPosition(-0.5)  # Retract elbow slightly
wait_until_motor_reaches(arm_sensors[1], 0.0)

# Step 5: Move arm to the right to place the box
# Step 5: Move arm to the left to place the box
print("Moving arm to the left to place the box...")
arm_motors[2].setPosition(-0.8)  # Extend elbow to the left
arm_motors[0].setPosition(1.57)  # Rotate base to the left (change to positive value)
wait_until_motor_reaches(arm_sensors[0], 1.57)  # Wait for base rotation to complete

# Step 6: Lower the arm to place the box
print("Lowering the arm to place the box...")
arm_motors[1].setPosition(-0.4)  # Lower shoulder
arm_motors[3].setPosition(-1.0)  # Adjust wrist for placing
wait_until_motor_reaches(arm_sensors[1], -0.4)
...
# Step 8: Return the arm to resting position
print("Returning arm to resting position...")
arm_motors[1].setPosition(0.0)  # Lift shoulder back up
arm_motors[2].setPosition(0.0)  # Retract elbow
arm_motors[3].setPosition(0.0)  # Reset wrist position
arm_motors[0].setPosition(0.0)  # Reset base to forward position
wait_until_motor_reaches(arm_sensors[1], 0.0)
